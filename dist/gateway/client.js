...
let directGatewayCall = null;
let ingestRuntimeStatus = {
...
const knownModelByAgent = new Map();
const knownModelBySession = new Map();
const KNOWN_MODEL_CACHE_LIMIT = 2000;
const policyStats = {
    stored: 0,
    dropped: 0,
    droppedByReason: new Map(),
    storedByType: new Map(),
    droppedByType: new Map()
};
...
export function getIngestPolicyStats() {
    const mapToObj = (m) => Object.fromEntries(Array.from(m.entries()).sort((a, b) => b[1] - a[1]));
    return {
...
async function startGatewayClient() {
    ...
    if (modulePath) {
        ...
        GatewayClient =
            mod.GatewayClient ||
                Object.values(mod).find((v) => typeof v === 'function' && v.prototype && typeof v.prototype.start === 'function' && typeof v.prototype.request === 'function');
    }
    if (!GatewayClient) {
...
function maybeEmitSessionOutputFromGateway(eventName, payload, frame) {
...
function handleGatewayEvent(frame) {
    ...
    if (!eventName || eventName === 'tick' || eventName === 'connect.challenge')
        return;
...
function normalizeGatewayFrame(eventName, payload) {
    const type = eventName;
    let domain = 'system';
    let severity = 'info';
    let summary = `Event: ${eventName}`;
    if (eventName.startsWith('model.') || eventName.startsWith('agent.'))
        domain = 'agent';
    else if (eventName.startsWith('tool.'))
        domain = 'tool';
    else if (eventName.startsWith('message.') || eventName.startsWith('webhook.') || eventName.startsWith('channel.'))
        domain = 'channel';
    else if (eventName.startsWith('session.') || eventName.startsWith('queue.lane.'))
        domain = 'session';
    else if (eventName.startsWith('access.'))
        domain = 'access';
    if (eventName.includes('error') || payload?.level === 'error')
        severity = 'error';
    if (eventName.includes('warn') || payload?.level === 'warn')
        severity = 'warning';
    switch (eventName) {
        case 'model.usage':
...
function buildKnownModelEntry(model, provider, updatedAtMs) {
    const entry = { updatedAtMs };
    if (model)
        entry.model = model;
    if (provider)
...
function cacheKnownModelFromEvent(e, nowMs) {
    ...
    if (e?.agentId) {
        knownModelByAgent.set(String(e.agentId), buildKnownModelEntry(model ?? currentByAgent?.model, provider ?? currentByAgent?.provider, nowMs));
        trimModelCache(knownModelByAgent);
    }
    if (e?.sessionKey) {
        knownModelBySession.set(String(e.sessionKey), buildKnownModelEntry(model ?? currentBySession?.model, provider ?? currentBySession?.provider, nowMs));
        trimModelCache(knownModelBySession);
    }
}
function enrichEventWithKnownModel(e) {
    const payload = e?.data;
    if (!payload || typeof payload !== 'object' || Array.isArray(payload))
        return e;
    ...
    if (!hasModel && known.model)
        nextPayload.model = known.model;
    if (!hasProvider && known.provider)
        nextPayload.provider = known.provider;
    return { ...e, data: nextPayload };
}
function countMapInc(map, key) {
    map.set(key, (map.get(key) || 0) + 1);
}
function policyDrop(reason, type) {
    policyStats.dropped += 1;
    countMapInc(policyStats.droppedByReason, reason);
    countMapInc(policyStats.droppedByType, type);
}
function policyStore(type) {
    policyStats.stored += 1;
    countMapInc(policyStats.storedByType, type);
}
function shouldStoreNormalized(e, nowMs) {
    const type = String(e?.type || 'unknown.event');
    if (!isEventTypeEnabled(type))
        return { store: false, reason: 'disabled-by-toggle' };
    try {
...
function resolveOpenClawDistDir() {
    ...
    try {
        ...
        if (bin) {
            ...
            if (fs.existsSync(candidate2))
                return fs.realpathSync(candidate2);
        }
    }
    catch { /* fall through */ }
    ...
    try {
        ...
        if (npmRoot)
            candidates.unshift(path.join(npmRoot, 'openclaw', 'dist'));
    }
    catch { /* ignore */ }
...
async function callGatewayDirect(method, params = {}) {
    ...
    if ((token || password) && GATEWAY_URL)
        opts.url = GATEWAY_URL;
...
function findOpenClawClientModule() {
    const distDir = getOpenClawDistDir();
    if (!distDir)
        return null;
    ...
    for (const entry of candidates) {
        ...
        try {
            ...
            if (!source.includes('GatewayClient'))
                continue;
            const looksLikeGatewayClient = (source.includes('onHelloOk') && source.includes('onEvent')) ||
                source.includes('observe.subscribe') ||
                source.includes('queueConnect');
            if (!looksLikeGatewayClient)
                continue;
            return modulePath;
        }
        catch {
...
function startFallbackIngest(reason) {
...
function startSessionEnrichment(mode) {
    if (sessionPollTimer)
        return;
    ...
    for (const file of files) {
        try {
            ...
            for (const line of recent)
                processLine(file, line, { inferThinking: false });
    ...
    for (const file of files) {
        try {
            ...
            for (const line of lines)
                processLine(file, line, { inferThinking: true });
...
function inferNetworkTargets(obj) {
    const found = [];
    const seen = new Set();
    const walk = (v) => {
        if (v == null)
...
function inferExecCommandTargets(args) {
    const command = String(args?.command || args?.cmd || '').trim();
    if (!command)
        return [];
    const lower = command.toLowerCase();
    let commandClass = '';
    if (/(^|\s)curl(\s|$)/.test(lower))
        commandClass = 'curl';
    else if (/(^|\s)wget(\s|$)/.test(lower))
        commandClass = 'wget';
    else if (/(^|\s)http(\s|$)/.test(lower))
        commandClass = 'httpie';
    else if (/(^|\s)(python|python3)(\s|$)/.test(lower) && /(requests|urllib|httpx)/.test(lower))
        commandClass = 'python-http';
    else if (/(^|\s)node(\s|$)/.test(lower) && /(fetch\s*\(|https?\.request\s*\()/.test(lower))
        commandClass = 'node-http';
    if (!commandClass)
        return [];
...
function inferAgentIdFromSessionPath(file) {
    ...
    if (idx > 0)
        return parts[idx - 1] || 'main';
    return 'main';
}
function processLine(file, line, opts) {
    let row;
    try {
        row = JSON.parse(line);
    }
    ...
    const textContent = contentParts.filter((part) => part?.type === 'text').map((part) => String(part?.text || '')).join('\n');
    ...
    if (role === 'assistant') {
        ...
        const hasAssistantText = content.some((part) => part?.type === 'text' && typeof part?.text === 'string' && part.text.trim().length > 0);
