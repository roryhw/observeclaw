...
export type RedactionMode = 'best-effort' | 'allowlist';

export function redactObject(input: any, mode: RedactionMode = 'best-effort'): any {
  if (input === null || input === undefined) return input;

  if (typeof input === 'string') {
    if (/bearer\s+[a-z0-9\-_.=]+/i.test(input)) return '***REDACTED***';
    if (input.length > 80 && /[A-Za-z0-9_\-]{40,}/.test(input)) return '***REDACTED***';
    return input;
  }

  if (Array.isArray(input)) return input.map((x) => redactObject(x, mode));
