...
import { redactObject, type RedactionMode, containsSecretLikeString } from './security/redaction.js';
...
function buildSessionCookie(token: string, secure = false) {
...
function requestHasValidSessionCookie(request: any) {
  const cookies = parseCookies(request?.headers?.cookie || '');
  return verifySessionToken(cookies[SESSION_COOKIE_NAME]);
}
const REDACTION_MODE = (process.env.OBSERVECLAW_REDACTION_MODE || 'best-effort') as RedactionMode;
...
function resolveAgentRoots(agentId?: string): string[] {
...
function listAgents(): Array<{ id: string; name: string; workspace: string }> {
  const out: Array<{ id: string; name: string; workspace: string }> = [];
...
function sanitizeSoulPath(inputPath: string): string {
  const p = String(inputPath || '').trim();
  const allowed = new Set(['SOUL.md', 'MEMORY.md', 'IDENTITY.md']);
  if (!allowed.has(p)) throw new Error('Path not allowed');
  return p;
}
...
async function asyncExecJson(cmd: string, timeoutMs = 8000): Promise<any> {
  try {
...
async function asyncExecText(cmd: string, timeoutMs = 8000): Promise<string> {
  try {
    const parts = cmd.split(/\s+/);
    const { stdout } = await execFileAsync(parts[0]!, parts.slice(1), {
      encoding: 'utf8',
...
const _asyncCache = new Map<string, { data: any; ts: number; inflight?: Promise<any> }>();

async function cachedAsyncExecJson(cmd: string, cacheSec: number, timeoutMs = 8000): Promise<any> {
  const entry = _asyncCache.get(cmd);
  const now = Date.now();
  if (entry && now - entry.ts < cacheSec * 1000) return entry.data;
  ...
  const promise = asyncExecJson(cmd, timeoutMs).then((result) => {
    ...
    if (stale) { delete stale.inflight; }
...
async function cachedAsyncExecText(cmd: string, cacheSec: number, timeoutMs = 8000): Promise<string> {
  const entry = _asyncCache.get(cmd);
  const now = Date.now();
  if (entry && now - entry.ts < cacheSec * 1000) return entry.data;
  if (entry?.inflight) return entry.inflight;
...
function safeExecJson(cmd: string, timeoutMs = 8000): any {
  try {
...
async function getRuntimeConfigSummary(): Promise<any> {
...
function detectCapabilities(files: string[]): string[] {
  ...
  if (files.includes('package.json')) caps.push('Node');
  if (files.some(f => f.endsWith('.py'))) caps.push('Python');
  if (files.some(f => f.toLowerCase().includes('cron'))) caps.push('Scheduled');
  return caps;
}
...
async function analyzeProject(projectPath: string): Promise<any> {
  ...
  try {
    ...
    if (statusText.includes(path.basename(projectPath))) info.isRunning = true;
  } catch {}
...
function getWorkspaceRoots(): string[] {
  ...
  try {
    if (config?.agents?.defaults?.workspace) roots.add(config.agents.defaults.workspace);
    if (Array.isArray(config?.agents?.list)) {
      for (const a of config.agents.list) {
        if (a?.workspace) roots.add(String(a.workspace));
      }
    }
  } catch {}
...
function writeAudit(action: string, target: string, status: string, details: any, actor?: string) {
  try {
    db.prepare(`
...
const pulseSeenEventIds: string[] = [];
const pulseSeenEventSet = new Set<string>();
const retentionTriggerSeen = new Set<string>();
const PULSE_DEBUG = process.env.OBSERVECLAW_PULSE_DEBUG === '1';
const jobRunLocks = new Set<string>();

async function runJobIfIdle(name: string, fn: () => void | Promise<void>) {
  if (jobRunLocks.has(name)) return false;
...
function markPulseSeen(eventId?: string) {
  const id = String(eventId || '');
  if (!id || pulseSeenEventSet.has(id)) return;
  pulseSeenEventSet.add(id);
  pulseSeenEventIds.push(id);
  if (pulseSeenEventIds.length > 50000) {
    const drop = pulseSeenEventIds.shift();
    if (drop) pulseSeenEventSet.delete(drop);
  }
}

function hasPulseSeen(eventId?: string): boolean {
  const id = String(eventId || '');
  return !!id && pulseSeenEventSet.has(id);
}

function maybeHandleRetentionTrigger(row: any) {
  const id = String(row?.id || '');
  if (!id || retentionTriggerSeen.has(id)) return;
  if (String(row?.type || '') !== 'cron') return;
  ...
  if (!marker.includes('OBSERVECLAW_RETENTION_RUN')) return;
...
function pollRetentionCronTriggers() {
  try {
    const rows = db.prepare(`
      ...
      ORDER BY rowid ASC
      LIMIT 200
    `).all(retentionCronLastRowId) as any[];

    if (!rows.length) return;
    for (const r of rows) {
      maybeHandleRetentionTrigger(r);
      const rid = Number(r?._rowid || 0);
      if (rid > retentionCronLastRowId) retentionCronLastRowId = rid;
    }
  } catch {}
}
...
function bumpNetworkPolicyDrop(reason: NetworkExcludeReason) {
...
const getOpenClawVersion = (() => {
  ...
  return async (): Promise<string> => {
    const now = Date.now();
    if (cached && now - fetchedAt < 5 * 60 * 1000) return cached;
    try {
...
const compareDottedVersions = (a: string | null, b: string | null) => {
  if (!a || !b) return 0;
...
async function fetchLatestOpenClawVersion() {
  ...
  if (!latest) throw new Error('latest version missing from npm response');
...
async function getPersistedOpenClawVersionStatus() {
  ...
  const existing = db.prepare('SELECT * FROM version_checks WHERE checkDate = ?').get(checkDate) as any;
...
const getOpenClawGatewayProcessInfo = async (): Promise<{ pid: number | null; uptimeSeconds: number }> => {
  try {
    ...
    if (best) return { pid: best.pid, uptimeSeconds: best.uptimeSeconds };
  } catch { /* fall through */ }
  return { pid: null, uptimeSeconds: Math.round(process.uptime()) };
};

const getOpenClawGatewayUptime = async (): Promise<number> => (await getOpenClawGatewayProcessInfo()).uptimeSeconds;

const getPrimaryLocalIp = (): string | null => {
  try {
...
let _storageCache: { ts: number; result: { free: number; total: number; used: number; percentUsed: number } } | null = null;
const STORAGE_CACHE_MS = 30_000; // refresh every 30s

const getStorageStatus = () => {
  if (_storageCache && Date.now() - _storageCache.ts < STORAGE_CACHE_MS) return _storageCache.result;
...
server.get('/api/glance', async (request: any) => {
  ...
  const openAlerts = (db.prepare('SELECT COUNT(*) as c FROM alerts WHERE acknowledged = 0').get() as any)?.c || 0;
  ...
  const cronEnabled = cronJobs.filter((job: any) => !!job?.enabled).length;
  const enabledCronJobs = cronJobs.filter((job: any) => !!job?.enabled);
  const nextCronRunMs = enabledCronJobs
    .map((job: any) => Number(job?.state?.nextRunAtMs || 0))
  ...
  `).all(startSevenDays.toISOString()) as any[];

  const parseExternalLabel = (row: any) => {
    ...
    for (const candidate of candidates) {
      ...
      if (candidate === 'network') continue;
      return candidate.replace(/^https?:\/\//, '').replace(/\/.*$/, '');
    }
    return '';
  };

  const isExternalLabel = (label: string) => {
    const v = String(label || '').toLowerCase();
    if (!v) return false;
    if (v === 'localhost' || v === '127.0.0.1' || v === '::1') return false;
    if (v.startsWith('192.168.') || v.startsWith('10.') || /^172\.(1[6-9]|2\d|3[0-1])\./.test(v)) return false;
    return true;
  };

  const normalizedExternalRows = externalRows.map((row: any) => ({ ...row, _label: parseExternalLabel(row) })).filter((row: any) => isExternalLabel(row._label));

  const perDay = dayStarts.map((start, idx) => {
    const end = new Date(start);
    end.setDate(end.getDate() + 1);
    const items = normalizedExternalRows.filter((row: any) => row.timestamp >= start.toISOString() && row.timestamp < end.toISOString());
    const destinations = new Set(items.map((row: any) => String(row._label || '').trim()).filter(Boolean));
    return {
  ...
  const activityDaily = dayStarts.map((start, idx) => {
    ...
    const toolCalls = (db.prepare('SELECT COUNT(*) as c FROM normalized_events WHERE type = ? AND timestamp >= ? AND timestamp < ?').get('tool.invoke', startIso, endIso) as any)?.c || 0;
    const modelCalls = (db.prepare('SELECT COUNT(*) as c FROM normalized_events WHERE type = ? AND timestamp >= ? AND timestamp < ?').get('model.usage', startIso, endIso) as any)?.c || 0;
    const conversationTurns = (db.prepare('SELECT COUNT(*) as c FROM normalized_events WHERE type = ? AND timestamp >= ? AND timestamp < ?').get('message.processed', startIso, endIso) as any)?.c || 0;
    const shellCmds = (db.prepare("SELECT COUNT(*) as c FROM normalized_events WHERE type = 'tool.invoke' AND json_extract(data, '$.tool') = 'exec' AND timestamp >= ? AND timestamp < ?").get(startIso, endIso) as any)?.c || 0;
    const fileMutations = (db.prepare("SELECT COUNT(*) as c FROM normalized_events WHERE type = 'tool.invoke' AND json_extract(data, '$.tool') IN ('edit', 'write') AND timestamp >= ? AND timestamp < ?").get(startIso, endIso) as any)?.c || 0;
    const webResearch = (db.prepare("SELECT COUNT(*) as c FROM normalized_events WHERE type = 'tool.invoke' AND json_extract(data, '$.tool') IN ('web_search', 'web_fetch') AND timestamp >= ? AND timestamp < ?").get(startIso, endIso) as any)?.c || 0;
    const browserActions = (db.prepare("SELECT COUNT(*) as c FROM normalized_events WHERE type = 'tool.invoke' AND json_extract(data, '$.tool') = 'browser' AND timestamp >= ? AND timestamp < ?").get(startIso, endIso) as any)?.c || 0;
    return {
      label: idx === 6 ? 'TODAY' : start.toLocaleDateString([], { weekday: 'short' }).toUpperCase(),
      shortLabel: idx === 6 ? 'TODAY' : start.toLocaleDateString([], { weekday: 'short' }).slice(0, 3).toUpperCase(),
      toolCalls,
...
server.get('/api/coverage', async (request) => {
  ...
  const domainRows = db.prepare(`SELECT domain, COUNT(*) as count FROM normalized_events GROUP BY domain`).all() as any[];
  const typeRows = db.prepare(`SELECT type, COUNT(*) as count FROM normalized_events GROUP BY type`).all() as any[];

  const domainMap: Record<string, number> = Object.fromEntries(domainRows.map((r: any) => [r.domain, Number(r.count)]));
  const typeMap: Record<string, number> = Object.fromEntries(typeRows.map((r: any) => [r.type, Number(r.count)]));

  const response = {
    totals: {
      rawEvents: (db.prepare('SELECT COUNT(*) as c FROM raw_events').get() as any)?.c || 0,
      normalizedEvents: (db.prepare('SELECT COUNT(*) as c FROM normalized_events').get() as any)?.c || 0
    },
...
server.get('/api/log-explorer/facets', async (request) => {
  const distinct = (col: string, limit = 200) =>
    db.prepare(`SELECT ${col} as v FROM normalized_events WHERE ${col} IS NOT NULL AND ${col} <> '' GROUP BY ${col} ORDER BY COUNT(*) DESC LIMIT ?`).all(limit).map((r: any) => r.v);
  ...
  writeAudit('read.log_explorer_facets', '/api/log-explorer/facets', 'ok', {}, String((request as any).ip));
  return { data: facets };
});

server.get('/api/log-explorer/favorites', async (request: any) => {
  const rows = db.prepare('SELECT * FROM log_explorer_favorites ORDER BY position ASC, updatedAt DESC').all() as any[];
  return { data: rows.map((r) => ({ ...r, query: (() => { try { return JSON.parse(r.queryJson || '{}'); } catch { return {}; } })() })) };
});

server.post('/api/log-explorer/favorites', async (request: any, reply) => {
  const body = request.body || {};
  const name = String(body.name || '').trim().slice(0, 48);
  const query = body.query || {};
  if (!name) return reply.code(400).send({ error: 'Name required' });

  const count = (db.prepare('SELECT COUNT(*) as c FROM log_explorer_favorites').get() as any)?.c || 0;
  if (count >= 5) return reply.code(400).send({ error: 'Max 5 favorites' });
  ...
  if (isDefault) db.prepare('UPDATE log_explorer_favorites SET isDefault = 0').run();
...
server.patch('/api/log-explorer/favorites/:id', async (request: any, reply) => {
  const id = String(request.params?.id || '');
  const body = request.body || {};
  const row = db.prepare('SELECT * FROM log_explorer_favorites WHERE id = ?').get(id) as any;
...
server.delete('/api/log-explorer/favorites/:id', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
server.get('/api/log-explorer', async (request) => {
  ...
  if (sessionKey) { where.push('sessionKey = ?'); params.push(sessionKey); }
  ...
  const total = (db.prepare('SELECT COUNT(*) as c' + base).get(...params) as any)?.c || 0;
  const rows = db.prepare('SELECT *' + base + ' ORDER BY timestamp DESC LIMIT ? OFFSET ?').all(...params, pageSize, offset) as any[];
  ...
  writeAudit('read.log_explorer', '/api/log-explorer', 'ok', { page, pageSize, total }, String((request as any).ip));
  return { data: sanitized, page, pageSize, total };
});

server.get('/api/log-explorer/raw/:rawEventId', async (request: any, reply) => {
  const rawEventId = String(request.params?.rawEventId || '');
  if (!rawEventId) return reply.code(400).send({ error: 'rawEventId required' });
  const row = db.prepare('SELECT * FROM raw_events WHERE id = ?').get(rawEventId) as any;
  if (!row) return reply.code(404).send({ error: 'Not found' });
  let payload: any = {};
...
server.get('/api/log-explorer/export.csv', async (request: any, reply) => {
  const q: any = request.query || {};
  ...
  const where: string[] = [];
  const params: any[] = [];
  const addEq = (k: string, col: string) => { if (q[k]) { where.push(`${col} = ?`); params.push(String(q[k])); } };
  if (q.from) { where.push('timestamp >= ?'); params.push(String(q.from)); }
  if (q.to) { where.push('timestamp <= ?'); params.push(String(q.to)); }
  addEq('domain', 'domain'); addEq('type', 'type'); addEq('severity', 'severity'); addEq('agentId', 'agentId'); addEq('sessionKey', 'sessionKey'); addEq('channel', 'channel');
  if (q.q) {
    const deep = String(q.deep || '0') === '1';
    const qExclude = String(q.qExclude || '0') === '1';
    if (deep) { where.push(qExclude ? 'NOT (summary LIKE ? OR type LIKE ? OR data LIKE ?)' : '(summary LIKE ? OR type LIKE ? OR data LIKE ?)'); params.push(`%${q.q}%`, `%${q.q}%`, `%${q.q}%`); }
    else { where.push(qExclude ? 'NOT (summary LIKE ? OR type LIKE ?)' : '(summary LIKE ? OR type LIKE ?)'); params.push(`%${q.q}%`, `%${q.q}%`); }
  }
  let sql = 'SELECT timestamp, domain, type, severity, agentId, sessionKey, channel, summary FROM normalized_events';
  if (where.length) sql += ' WHERE ' + where.join(' AND ');
  sql += ' ORDER BY timestamp DESC';
  if (mode === 'page') sql += ` LIMIT ${pageSize} OFFSET ${(page - 1) * pageSize}`;
  else sql += ` LIMIT ${maxRows}`;
...
server.post('/api/auth/logout', async (request: any, reply) => {
...
server.get('/api/auth/status', async (request: any) => {
  return { authorized: !!request.ocAuthorized, mode: request.ocAuthMode || null };
});

server.get('/api/retention/policy', async (request: any) => {
...
server.put('/api/retention/policy', async (request: any, reply) => {
  try {
    const body = request.body || {};
...
server.post('/api/retention/dry-run', async (request: any, reply) => {
  try {
    const policy = loadRetentionPolicy(db);
...
server.post('/api/retention/run', async (request: any, reply) => {
  const runId = ulid();
...
server.get('/api/retention/runs', async (request: any) => {
...
server.post('/api/retention/schedule', async (request: any, reply) => {
  const policy = loadRetentionPolicy(db);
  ...
  try {
    ...
    const existing = jobs.find((j: any) => String(j?.name || '') === 'observeclaw-retention');
...
server.get('/api/agents', async (request) => {
  ...
  writeAudit('read.agents', '/api/agents', 'ok', { count: agents.length }, String((request as any).ip));
  return { data: agents };
});

server.get('/api/workspaces/cron', async (request: any) => {
...
server.post('/api/workspaces/cron/:id/toggle', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
server.post('/api/workspaces/cron/:id/run', async (request: any, reply) => {
  const id = String(request.params?.id || '');
  try {
...
server.get('/api/workspaces/projects', async (request: any) => {
  ...
  for (const wsPath of workspaceRoots) {
    ...
    if (workspaceName === 'workspace') workspaceName = 'Main Workspace';
    ...
    try {
      ...
      for (const pth of scanDirs) {
        try {
          ...
          const matchedCron = cronJobs.find((job: any) => {
          ...
          if (matchedCron) {
            ...
            if (!info.capabilities.includes('Scheduled')) info.capabilities.unshift('Scheduled');
          }
  ...
  const out = Array.from(dedup.values()).sort((a: any, b: any) => {
    if (a.workspaceName !== b.workspaceName) return String(a.workspaceName).localeCompare(String(b.workspaceName));
...
function getSessionTypeInfo(sessionKey: string): 'main'|'cron'|'subagent'|'other' {
  const key = String(sessionKey || '');
  if (key.endsWith(':main') && !key.includes(':cron:') && !key.includes(':subagent:') && !key.includes(':spawn:')) return 'main';
  if (key.includes(':cron:')) return 'cron';
  if (key.includes(':subagent:') || key.includes(':spawn:')) return 'subagent';
  return 'other';
}

server.get('/api/conversations', async (request: any, reply) => {
  ...
  try {
    ...
    if (!fs.existsSync(indexPath)) return { data: [] };

    const index = JSON.parse(fs.readFileSync(indexPath, 'utf8')) || {};
    const items: any[] = [];

    for (const [key, s] of Object.entries(index as any)) {
      const sess: any = s || {};
      ...
      let fileSize = 0;
      try { fileSize = fs.existsSync(filePath) ? fs.statSync(filePath).size : 0; } catch {}
      ...
      if (channel && String(row.channel).toLowerCase() !== channel) continue;
      if (type && String(typeInfo) !== type) continue;
      if (query) {
...
server.get('/api/conversations/:sessionId', async (request: any, reply) => {
      ...
      try {
        ...
        if (entry?.type === 'message' && entry?.message) {
          ...
          if (role === 'user' || role === 'assistant' || role === 'system') {
            ...
            if (text) messages.push({ role, content: String(text).slice(0, 20000), timestamp: ts, collapsed: role === 'system' });
          }
        } else if (entry?.type === 'summary' || entry?.summary) {
          const txt = String(entry?.summary || entry?.text || '').trim();
          if (txt) messages.push({ role: 'system', content: txt.slice(0, 20000), timestamp: ts, collapsed: true });
        }
      } catch {}
    }
...
server.get('/api/artifacts/files', async (request: any, reply) => {
  const q = request.query || {};
  const agentId = String(q.agent || 'main');
...
server.get('/api/artifacts/file', async (request: any, reply) => {
  const q = request.query || {};
  const agentId = String(q.agent || 'main');
...
server.get('/api/artifacts/file/raw', async (request: any, reply) => {
  const q = request.query || {};
  const agentId = String(q.agent || 'main');
  const relPath = String(q.path || '').trim();
...
server.get('/api/file', async (request: any, reply) => {
  const q = request.query || {};
  const agentId = String(q.agent || 'main');
...
server.post('/api/file', async (request: any, reply) => {
  const body = request.body || {};
  ...
  try {
    ...
    if (!full.startsWith(ws)) return reply.code(400).send({ error: 'Invalid path' });
...
server.get('/api/audit/export', async (request, reply) => {
  ...
  let rows: any[];
  if (since) {
    rows = db.prepare('SELECT * FROM audit_events WHERE timestamp >= ? ORDER BY timestamp DESC LIMIT ?').all(since, limit) as any[];
  } else {
    rows = db.prepare('SELECT * FROM audit_events ORDER BY timestamp DESC LIMIT ?').all(limit) as any[];
  }
...
server.get('/api/security/redaction-check', async (request: any) => {
  const sampleRows = db.prepare('SELECT data FROM normalized_events ORDER BY timestamp DESC LIMIT 250').all() as any[];
  let scanned = 0;
  let suspect = 0;
  ...
  for (const row of sampleRows) {
    ...
    const redacted = JSON.stringify(redactObject((() => { try { return JSON.parse(raw); } catch { return raw; } })(), REDACTION_MODE));
    if (containsSecretLikeString(redacted)) {
      suspect += 1;
      if (examples.length < 5) examples.push(redacted.slice(0, 160));
    }
  }
...
server.get('/api/rules', async (request) => {
  ...
  writeAudit('read.rules', '/api/rules', 'ok', { count: rows.length }, String((request as any).ip));
  return { data: rows };
});

server.get('/api/rules/diagnostics', async (request) => {
  const rows = Array.from(ruleEvalDiagnostics.values()).sort((a: any, b: any) => String(b.evaluatedAt || '').localeCompare(String(a.evaluatedAt || '')));
  writeAudit('read.rules.diagnostics', '/api/rules/diagnostics', 'ok', { count: rows.length }, String((request as any).ip));
  return { data: rows };
});
...
server.get('/api/system/ingest-status', async (request) => {
  ...
  writeAudit('read.system.ingest_status', '/api/system/ingest-status', 'ok', { mode: data.ingest.mode }, String((request as any).ip));
  return data;
});

server.post('/api/rules', async (request: any, reply) => {
  const body = (request.body || {}) as any;
  const id = ulid();
  const name = String(body.name || '').trim();
  const kind = String(body.kind || '').trim();
  if (!name || !kind) return reply.code(400).send({ error: 'name and kind are required' });
...
server.patch('/api/rules/:id', async (request: any, reply) => {
  const id = String(request.params?.id || '');
  const body = (request.body || {}) as any;

  const fields: string[] = [];
  const params: any[] = [];
  if (typeof body.enabled === 'boolean') { fields.push('enabled = ?'); params.push(body.enabled ? 1 : 0); }
  if (typeof body.threshold === 'number') { fields.push('threshold = ?'); params.push(body.threshold); }
  if (typeof body.windowSec === 'number') { fields.push('windowSec = ?'); params.push(body.windowSec); }
  if (typeof body.dedupeSec === 'number') { fields.push('dedupeSec = ?'); params.push(body.dedupeSec); }
  if (typeof body.notifyEnabled === 'boolean') { fields.push('notifyEnabled = ?'); params.push(body.notifyEnabled ? 1 : 0); }
  if (typeof body.notifyChannel === 'string') { fields.push('notifyChannel = ?'); params.push(body.notifyChannel); }
  if (typeof body.notifyCooldownSec === 'number') { fields.push('notifyCooldownSec = ?'); params.push(body.notifyCooldownSec); }
  if (typeof body.name === 'string' && body.name.trim()) { fields.push('name = ?'); params.push(body.name.trim()); }
  if (typeof body.domain === 'string') { fields.push('domain = ?'); params.push(body.domain); }
  if (typeof body.type === 'string') { fields.push('type = ?'); params.push(body.type); }
  if (typeof body.kind === 'string' && body.kind.trim()) { fields.push('kind = ?'); params.push(body.kind.trim()); }

  if (!fields.length) return { ok: true, changes: 0 };
  params.push(id);
...
server.delete('/api/rules/:id', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
server.get('/api/alerts', async (request) => {
  ...
  const cond: string[] = [];
  const params: any[] = [];
  if (onlyOpen) cond.push('acknowledged = 0');
  if (severity !== 'all') {
    cond.push('severity = ?');
...
server.post('/api/alerts/:id/ack', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
server.post('/api/alerts/ack-all', async (request: any, reply) => {
...
server.get('/api/notifications/outbox', async (request) => {
  const q = (request as any).query || {};
  const limit = q.limit ? Math.min(500, parseInt(q.limit)) : 100;
  const rows = db.prepare('SELECT * FROM notifications_outbox ORDER BY timestamp DESC LIMIT ?').all(limit) as any[];
  writeAudit('read.notifications_outbox', '/api/notifications/outbox', 'ok', { count: rows.length }, String((request as any).ip));
  return { data: rows };
});

server.post('/api/notifications/:id/mark-sent', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
server.post('/api/notifications/:id/retry', async (request: any, reply) => {
  const id = String(request.params?.id || '');
...
function seedDefaultRules() {
  const existing = (db.prepare('SELECT COUNT(*) as c FROM alert_rules').get() as any)?.c || 0;
...
function ensureExternalOutboundRule() {
  try {
    const existing = db.prepare("SELECT id FROM alert_rules WHERE kind = 'network_connection_rate' LIMIT 1").get() as any;
    if (existing?.id) {
      db.prepare(`
        UPDATE alert_rules
...
function ensureStrictNetworkRule() {
  try {
    const existing = db.prepare("SELECT id FROM alert_rules WHERE kind = 'network_strict_violation' LIMIT 1").get() as any;
...
function ensureSocketConnectRule() {
  try {
    const existing = db.prepare("SELECT id FROM alert_rules WHERE kind = 'network_connect_rate' LIMIT 1").get() as any;
    if (existing?.id) {
      db.prepare(`
        UPDATE alert_rules
...
function upsertAlert(ruleId: string, severity: string, title: string, details: any, dedupeKey: string, dedupeSec: number = 600): { alertId: string | null; deduped: boolean } {
  const existing = db.prepare('SELECT id FROM alerts WHERE dedupeKey = ? AND timestamp >= ?').get(
    dedupeKey,
    new Date(Date.now() - Math.max(30, dedupeSec) * 1000).toISOString()
  ) as any;
  if (existing?.id) return { alertId: null, deduped: true };
...
function buildNotificationBody(title: string, severity: string, details: any): string {
  const lines: string[] = [];
  ...
  if (details?.count != null && details?.windowSec != null) lines.push(`count=${details.count} in ${details.windowSec}s`);
  if (Array.isArray(details?.topHosts) && details.topHosts.length) {
    const top = details.topHosts.slice(0, 3).map((h: any) => `${h.host}(${h.count})`).join(', ');
    lines.push(`top hosts: ${top}`);
  }
  if (Array.isArray(details?.topPorts) && details.topPorts.length) {
    const ports = details.topPorts.slice(0, 3).map((p: any) => `${p.port}(${p.count})`).join(', ');
    lines.push(`top ports: ${ports}`);
  }
  if (Array.isArray(details?.agents) && details.agents.length) lines.push(`agents: ${details.agents.join(', ')}`);
  if (details?.sourceType) lines.push(`sourceType=${details.sourceType}`);
  if (details?.sourceFilter) lines.push(`sourceFilter=${details.sourceFilter}`);
...
function queueNotificationIfNeeded(rule: any, alertId: string, severity: string, title: string, details: any): { queued: boolean; reason: string; channel?: string } {
  if (!rule?.notifyEnabled) return { queued: false, reason: 'notify-disabled' };
  const cooldownSec = Number(rule.notifyCooldownSec || 900);
  const last = rule.lastNotifiedAt ? new Date(rule.lastNotifiedAt).getTime() : 0;
  const now = Date.now();
  if (last && now - last < cooldownSec * 1000) return { queued: false, reason: 'notify-cooldown' };
...
async function processNotificationOutbox() {
  const rows = db.prepare("SELECT * FROM notifications_outbox WHERE status='pending' ORDER BY timestamp ASC LIMIT 10").all() as any[];
...
function sqlLikeMatch(value: string, likePattern: string): boolean {
...
function normalizeNetworkSourceClass(data: any): 'semantic' | 'socket' | 'unknown' {
  const source = String(data?.source || '').toLowerCase();
  if (source.includes('socket') || data?.sourceType === 'socket-observed') return 'socket';
  if (data?.tool || source.includes('tool') || source.includes('semantic') || source.includes('exec')) return 'semantic';
  return 'unknown';
}

function getFilteredNetworkRows(
  since: string,
  eventType: 'network.request' | 'network.connect',
  opts?: { proto?: string; hostLikePattern?: string; sourceClass?: 'semantic' | 'socket' | 'any' }
): any[] {
  const rows = db.prepare(`
  ...
  return rows.map((row: any) => {
    ...
    if (!isPublicHost(row.host)) return false;
    if (protocol && row.protocol !== protocol) return false;
    if (!sqlLikeMatch(row.host, hostLikePattern)) return false;
    if (sourceClass !== 'any' && row.sourceClass !== sourceClass) return false;
    const policy = evaluateNetworkPolicy(NETWORK_POLICY, 'outbound', row.host);
    return policy.allow;
  });
}

function getNetworkNoveltyKeys(row: any): string[] {
  ...
  if (remoteIp) keys.add(remoteIp);
  if (resolvedIp) keys.add(resolvedIp);
  return Array.from(keys);
}

function getSeenHostsBefore(windowStartIso: string, noveltyDays: number, eventType: 'network.request' | 'network.connect'): Set<string> {
  ...
  const rows = db.prepare(`
    ...
    WHERE timestamp >= ?
      ...
      AND domain = 'network'
      AND type = ?
  `).all(lookbackStartIso, windowStartIso, eventType) as any[];

  const out = new Set<string>();
  for (const r of rows) {
    try {
      const d = JSON.parse(r.data || '{}');
      for (const key of getNetworkNoveltyKeys(d)) out.add(key);
    } catch {}
  }
  return out;
}

function summarizeNetworkRows(rows: any[], since: string, windowSec: number, extra: Record<string, any> = {}) {
  ...
  for (const row of rows) {
    ...
    if (Number.isFinite(row.port) && row.port > 0) portCounts.set(row.port, (portCounts.get(row.port) || 0) + 1);
    if (row.agentId) agentCounts.set(row.agentId, (agentCounts.get(row.agentId) || 0) + 1);
  ...
  const samples = rows.slice(0, 5).map((r: any) => ({
...
const ruleEvalDiagnostics = new Map<string, any>();

function resolveRuleNetworkSourceFilter(rule: any, fallback: 'semantic' | 'socket' | 'any' = 'any'): 'semantic' | 'socket' | 'any' {
  const raw = String(rule?.domain || '').trim().toLowerCase();
  if (raw === 'semantic' || raw === 'socket' || raw === 'any') return raw;
  return fallback;
}
...
function evaluateRules() {
  ...
  for (const r of rules) {
    ...
    let details: any = { since, windowSec };

    if (r.kind === 'error_burst') {
      const count = (db.prepare(`SELECT COUNT(*) as c FROM normalized_events WHERE timestamp >= ? AND (severity = 'error' OR type LIKE '%error%')`).get(since) as any)?.c || 0;
      details = { count, since };
      if (count >= (r.threshold || 3)) {
        ...
        if (alertResult.alertId) details.notification = queueNotificationIfNeeded(r, alertResult.alertId, 'warning', r.name, details);
        triggered = true;
      }
    } else if (r.kind === 'event_rate') {
      const count = (db.prepare(`SELECT COUNT(*) as c FROM normalized_events WHERE timestamp >= ? AND domain = ? AND type = ?`).get(since, r.domain, r.type) as any)?.c || 0;
      ...
      if (count >= (r.threshold || 20)) {
        ...
        details.deduped = alertResult.deduped;
        if (alertResult.alertId) details.notification = queueNotificationIfNeeded(r, alertResult.alertId, 'info', r.name, details);
        triggered = true;
      }
      ...
      const defaultSource: 'semantic' | 'socket' | 'any' = isConnectRule ? 'any' : 'semantic';
      ...
      const novelRows = rows.filter((row: any) => getNetworkNoveltyKeys(row).every((key) => !seenHosts.has(key)));
      ...
      details = summarizeNetworkRows(novelRows, since, windowSec, {
        ...
        attribution: isConnectRule
          ...
          : 'tool-context',
        novelHosts: Array.from(new Set(novelRows.map((x: any) => x.host))).slice(0, 20)
      });
      ...
      const strictRows = rows.filter((row: any) => {
      ...
      details = summarizeNetworkRows(strictRows, since, windowSec, {
        ...
        reasons: {
          non443: strictRows.filter((r: any) => Number(r.port || 0) > 0 && Number(r.port) !== 443).length,
          tlsError: strictRows.filter((r: any) => !!r.tlsError).length,
          badCert: strictRows.filter((r: any) => r.certValid === false).length
        }
      });
      details.eventCount = rows.length;
    ...
    if (ruleEvalDiagnostics.size > 200) {
      ...
      if (oldest) ruleEvalDiagnostics.delete(oldest);
    }
  }
}

function ingestEventIntoPulse(row: any) {
  try {
    const eventId = String(row?.id || '');
    if (hasPulseSeen(eventId)) return;
...
function hydratePulseFromDatabase() {
  try {
    ...
    for (const r of inOrder) ingestEventIntoPulse(r);

    const max = (db.prepare('SELECT COALESCE(MAX(rowid), 0) as m FROM normalized_events').get() as any)?.m || 0;
...
function syncPulseFromDatabase() {
  try {
    ...
    for (const r of rows) {
      ...
      maybeHandleRetentionTrigger(r);
      const rid = Number(r?._rowid || 0);
      if (rid > pulseLastRowId) pulseLastRowId = rid;
    }
  } catch {}
}

function runRetentionSweep(trigger: 'startup' | 'interval' | 'manual' = 'interval') {
  try {
    const policy = loadRetentionPolicy(db);
